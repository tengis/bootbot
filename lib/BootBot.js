...
class BootBot extends EventEmitter {
  ...
  sendRequest(body, endpoint, method) {
    ...
    const url = `https://graph.facebook.com/v2.6/me/${endpoint}?access_token=${this.accessToken}`;
    if (this.dashbot) {
      const requestId = this.dashbot.logOutgoing(Object.assign({url}, requestData));
    }
    ...
    .catch(err => {
      ...
      if (this.dashbot) {
        this.dashbot.logOutgoingResponse(requestId, err, null);
      }
    });
  }
