...
class BootBot extends EventEmitter {
  ...
  sendRequest(body, endpoint, method) {
    ...
    method = method || 'POST';
    const uri = `https://graph.facebook.com/v2.6/me/${endpoint}`
    const qs = {
      access_token: this.accessToken
    }
    var requestId;
    if (this.dashbot) {
      const dashBotData = { uri, qs, json: body, method };
      requestId = this.dashbot.logOutgoing(dashBotData);
    }
    return fetch(`${uri}?access_token=${qs.access_token}`, {
      method,
      headers: {
    ...
    .then(res => {
      if (this.dashbot) {
        this.dashbot.logOutgoingResponse(requestId, null, res.json());
      }
      return res.json()
    })
    .catch((err) => {
  ...
  setDashBotAnalytics(dashbot) {
    this.dashbot = dashbot;
  }

  addAdditionalEndpoint(route, callback) {
    this.app.use(route, callback);
  }

  addAdditionalSetting(name, value) {
